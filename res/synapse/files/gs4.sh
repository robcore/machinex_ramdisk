...
case "$1" in
	...
	DirGPUGovernor)
		...
		if [ "$(cat /sys/module/blu_plug/parameters/enabled)" -eq "1" ]; then
			BLU_PLUG=Active;
		else
			BLU_PLUG=Inactive;
		fi;
		...
		if [ "$(cat /sys/devices/virtual/misc/mako_hotplug_control/enabled)" -eq "1" ]; then
			MAKO_HOTPLUG=Active;
		else
			MAKO_HOTPLUG=INACTIVE;
		fi;
		...
		echo "Default HotPlug: $DEFAULT_HOTPLUG@nLazyplug: $LAZYPLUG@nMako_Hotplug: $MAKO_HOTPLUG@nBlu_Plug: $BLUPLUG@nMSM_Hotplug: $MSM_HOTPLUG@nIntelli_Hotplug: $INTELLI_HOTPLUG@nBricked_Hotplug: $BRICKED_HOTPLUG@nMSM_Sleeper: $MSM_SLEEPER@nAlucard_Hotplug: $ALUCARD_HOTPLUG"
	;;
	DirGPUMaxFrequency)
